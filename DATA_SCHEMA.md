...
| **Core Treatment Variable**<br>*Policy Exposure Capture* | `ETR` | **Effective Tax Rate (ETR)**: Calculated as $\text{Income Tax Expense} / \text{Pre-tax Income}$. Used to identify the baseline level of exposure to the global minimum tax shock. | Yahoo Finance / FMP | `yahooquery.Ticker.income_statement` (TaxProvision / PretaxIncome) |
...
| **Micro Control Variable (X)**<br>*Firm Financial Characteristics* | `Firm_Size` | **Firm Size**: Controlled using the natural logarithm of total assets, $\ln(\text{Total Assets})$, to eliminate systemic scale interference. | Yahoo Finance | `yahooquery.Ticker.balance_sheet` (TotalAssets) |
...
| **Macro Control Variable (X)**<br>*External Governance Environment* | `Gov_Quality` | **Regulatory Quality Index**: Controls for the institutional environment, governance quality, and compliance costs within core operating markets. | World Bank | `wbgapi` (Index: PV.EST / SG.REG.LLWS.XQ) |
